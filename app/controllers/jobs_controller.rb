# frozen_string_literal: true

class JobsController < ApplicationController
    before_action :role_auth, only: [:create, :update, :destroy]
    rescue_from ActiveRecord::RecordNotFound, with: :not_found

    #check if the job was successfully saved and return an error message if it wasn't
    def create
      job = Job.new(job_code: generate_code(12), job_name: params[:job_name], jobtag_code: params[:jobtag_code], employer_code: params[:employer_code])
      if job.save
        render json: job, status: :created
      else
        render json: { error: job.errors.full_messages }, status: :unprocessable_entity
      end
    end
  
    def index
      jobs = Job.all
      render json: jobs, status: :ok
    end
  
    def show
      job = find_job
      render json: job, status: :ok
    end
  
    # '/job/:id' edits an job of id in params(Update)
    #check if the job was successfully updated and return an error message if it wasn't
    def update
      job = find_job
      if job.update(job_params)
        render json: job, status: :ok
      else
        render json: { error: job.errors.full_messages }, status: :unprocessable_entity
      end
    end
  
    # allows searching for jobs based on tags and renders them
    def search
      jobs = Job.where(jobtag_code: params[:jobtag_code])
      render json: jobs, status: :ok
    end
  
    # '/job/:id' deletes a job of id in params(Delete by destroy)
    def destroy
      job = find_job
      job.destroy
      head :no_content
    end
  
    private
  
    def find_job
      Job.find(params[:id])
    end
  
    def job_params
      params.permit(:job_name, :jobtag_code, :employer_code)
    end
  
    def role_auth
      # Get token from request headers
      role = ["EMPLOYER", "ADMIN"]
      header = request.headers['Authorization']
      token = header.split(' ').last if header
      if token
        begin
          decoded_token = JWT.decode(token, Rails.application.secret_key_base)
          user_role = decoded_token[0]['role']
          unless role.include?(user_role)
            render json: { error: 'Unauthorized To Access Resource' }, status: :unauthorized
          end
        end
      end
    end
  
    def not_found
      render json: { error: 'Record not found' }, status: :not_found
    end
  end